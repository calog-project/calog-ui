      ...
      <div className="flex justify-center items-center gap-5 w-full">
        <div className="grid grid-cols-3 grid-rows-2 gap-[20px] w-full">
          {defaultCategories.map((category, index) => (
            <div
              key={index}
              className={`flex justify-center items-center w-full h-[70px] bg-white border border-gray-98 text-black-17 rounded-[10px] text-center font-medium gap-[5px] cursor-pointer`}>
              {category}
          ...
          {currentItems.map((category, index) => (
            <div
              key={index}
              className="relative flex justify-center items-center w-full h-[70px] bg-white border border-gray-98 text-black-17 rounded-[10px] text-center font-medium gap-[5px] cursor-pointer">
              {category}
              <button className="absolute right-4 top-1 text-black">
