...
export const getNotificationMessage = (type: NotificationType, meta?: NotificationMeta): string => {
  switch (type) {
    case NotificationType.FOLLOW_REQUESTED:
      return `${(meta as FollowRequestedMeta)?.followerNickname || '사용자'}님이 팔로우를 신청했습니다.`;
    case NotificationType.FOLLOWED:
      return `${(meta as FollowedMeta)?.followerNickname || '사용자'}님이 회원님을 팔로우했습니다.`;
    case NotificationType.SCHEDULE_INVITED:
      return `${(meta as ScheduleInvitedMeta)?.inviterNickname || '사용자'}님이 ${(meta as ScheduleInvitedMeta)?.scheduleTitle || '일정'}에 초대했습니다.\n아래 버튼을 눌러 참석 여부를 알려주세요.`;
    case NotificationType.SCHEDULE_UPCOMING: {
      const scheduleTitle = (meta as ScheduleUpcomingMeta)?.scheduleTitle;
      return scheduleTitle
        ? `'${scheduleTitle}' 일정이 곧 시작됩니다.\n자세한 내용은 캘린더에서 확인해 주세요.`
        : '일정이 곧 시작됩니다.\n자세한 내용은 캘린더에서 확인해 주세요.';
    }
    case NotificationType.SCHEDULE_SHARED:
      return `${(meta as ScheduleSharedMeta)?.inviterNickname || '사용자'}님이 일정을 공유했습니다.\n자세한 내용은 캘린더에서 확인해 주세요.`;
    case NotificationType.SCHEDULE_DELETED:
      return `${(meta as ScheduleDeletedMeta)?.scheduleTitle || '일정'}이 삭제되었습니다.`;
    default:
      return '새로운 알림이 있습니다.';
  }
};
